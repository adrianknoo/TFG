...
DATOS2 = readtable("Coordenadas_Asturias_Cantabria.xlsx"); % Lectura de coordenadas de helipuertos.

%% INPUTS: %%
Coordenadas_Ast_Cant = table2array(DATOS2(:,2:3)); % Vector Coordenadas (Latitud y Longitud).
lat = Coordenadas_Ast_Cant(:,1); % Vector Latitud.
lon = Coordenadas_Ast_Cant(:,2); % Vector longitud.
Ubicacion = table2cell(DATOS2(:,1));
...
for i = 1:numel(lat)
    geoshow(lat(i), lon(i), 'DisplayType', 'point', 'Marker', marcadores{mod(i,length(marcadores))+1},...
